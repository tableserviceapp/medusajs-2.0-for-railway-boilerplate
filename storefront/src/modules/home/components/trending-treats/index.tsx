        ...
        >
          {displayProducts.map((product, index) => {
            ...
            const productImage = isRealProduct 
              ? (product as HttpTypes.StoreProduct).thumbnail || (product as HttpTypes.StoreProduct).images?.[0]?.url || "https://images.unsplash.com/photo-1606313564200-e75d5e30476c?w=400&h=300&fit=crop"
                    ...
                    <div className="flex items-center justify-center space-x-2 mb-3">
                      <div className="flex items-center">
                        {[...Array(5)].map((_, i) => (
                          <svg
                            key={i}
                            className={`w-4 h-4 ${i < Math.floor(productRating) ? 'text-yellow-400' : 'text-gray-300'} fill-current`}
                            viewBox="0 0 20 20"
                          >
